...
def infor_formater(fav_infos, fav_str, mode, file_path, start_time, min_hot, print_level):
    ...
    # 写入到文件
    with open(file_path + "/format_blogs_info.t1.json", "w", encoding="utf-8", errors="ignore") as op:
        ...
        if blog_info["tags"]:
            ...
            if blog_info["key tag"] == "":
                ...
                if agg_non_prior_tag and prior_tags:
                    blog_info["key tag"] = "cn.net.hnkjxy"
                else:
        ...
        # 启用tag分类，启用优先tag
            ...
            # tag不在优先tag
                ...
                if agg_non_prior_tag:
                    article_path = file_path + "/article/cn.net.hnkjxy"
                # tag不在优先tag中，未启用非优先tag聚合
                else:
                    article_path = file_path + "/article/cn.net.hnkjxy/" + article_info["key tag"]
...
def save_img(imgs_info, file_path, img_save_info, classify_by_tag, prior_tags, agg_non_prior_tag, print_level):
    ...
    for img_info in imgs_info:
        ...
        for img_url in img_info["img urls"]:
            ...
            # 启用tag分类，启用优先tag
                ...
                # tag不在优先tag
                    ...
                    # tag不在优先tag中，启用非优先tag聚合
                    if agg_non_prior_tag:
                        img_path = file_path + "/img/cn.net.hnkjxy"
                    # tag不在优先tag中，未启用非优先tag聚合
                    else:
                        img_path = file_path + "/img/cn.net.hnkjxy/" + img_info["key tag"]
            # 文件名查重，保存
        ...
        if saved_num % 7 == 0 or saved_num == len(imgs_info):
            ...
            with open(file_path + "/img_save_info.t1.json", "w", encoding="utf-8") as i_op1:
...
def run(url, mode, save_mode, classify_by_tag, prior_tags, agg_non_prior_tag, login_info, start_time, tag_filt_num,
    ...
    if not os.path.exists(file_path + "/classified_blogs_info.t1.json") or not os.path.exists(
            file_path + "/format_blogs_info.t1.json"):
        if os.path.exists(file_path + "/img_save_info.t1.json"):
            print("找到阶段4进度文件，但阶段1或阶段2文件未找到，阶段4进度被删除")
            os.remove(file_path + "/img_save_info.t1.json")

    # 阶段2结束文件存在但阶段1结束文件不存在，删除阶段2文件
    if os.path.exists(file_path + "/classified_blogs_info.t1.json") and not os.path.exists(
            file_path + "/format_blogs_info.t1.json"):
        print("找到阶段2结束文件，但阶段1结束文件不存在，重置进度到阶段1")
        os.remove(file_path + "/classified_blogs_info.t1.json")

    # 自动整理项是否有改动，如果有改动进度重置到阶段2
    if save_mode["img"] and os.path.exists(file_path + "/img_save_info.t1.json"):
        auto_sort_setting = {"按tag分类": classify_by_tag, "优先tag": prior_tags,
                             "非优先tag聚合": agg_non_prior_tag}
        img_save_info_str = open(file_path + "/img_save_info.t1.json", "r", encoding="utf-8").read()
    ...
    if not os.path.exists(file_path + "/format_blogs_info.t1.json"):
        ...
        print("阶段1在之前的运行中已完成")
    blogs_info = json.loads(open(file_path + "/format_blogs_info.t1.json", encoding="utf-8").read())

    # 阶段2，分类，删除classified_fav_info.json以重启该阶段
    step2_start_time = time.time()
    print("\n" + "=====================" * 10 + "\n阶段2：博客分类，增加按自动整理选项增加需要的信息")
    if not os.path.exists(file_path + "/classified_blogs_info.t1.json"):
        blogs_info = update_key_tag(blogs_info, classify_by_tag, prior_tags, agg_non_prior_tag)
        classified_blogs = classify(blogs_info)

        with open(file_path + "/classified_blogs_info.t1.json", "w", encoding="utf-8", errors="ignore") as op:
        ...
        print("阶段2在之前的运行中已完成")
    classified_blogs = json.loads(open(file_path + "/classified_blogs_info.t1.json", "r", encoding="utf-8").read())
    ...
    if save_mode["img"]:
        ...
        if not os.path.exists(file_path + "/img_save_info.t1.json"):
            img_save_info = {"图片博客总数": len(classified_blogs["img"]), "已保存": 0, "自动整理设置": auto_sort_setting}
            print("新建保存进度 {}".format(img_save_info))
            with open(file_path + "/img_save_info.t1.json", "w", encoding="utf-8") as op1:
...
if __name__ == '__main__':
    # 基础设置  -------------------------------------------------------- #
    url = "http://www.lofter.com/tag/%E5%BA%B7%E7%BA%B3/month"
    ...
    prior_tags = ['社会学', '纯贱', '刺客信条乙女向', '乙女', "警探组", 'ezio/altair', 'haytham/connor', '现代组', '找船组', 'haytham/edward',
                  '海鲜组', 'hc', 'shaytham', '航海组', '茄面', '', 'ea', 'alte', 'ad钙奶好', 'ggpg', '暴卡', '垃圾上司组', 'adgg',
                  ...
                  'johnny depp', '约翰尼德普', '约翰尼·德普', '德普', '马库斯', '加百列', 'theseus', '刺客信条黑旗', 'ac3', '刺客信条3',
