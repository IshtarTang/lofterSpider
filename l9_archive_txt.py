...
import l4_archive_img_finished
...
def parse_archive_page(url, header, data, author_url, query_num, start_time, end_time, target_titles, merger_chapter):
    ...
    while True:
        ...
        page_data = l4_archive_img_finished.post_content(url=url, data=data, head=header)
        ...
        if start_time:
            if l4_archive_img_finished.is_stamp_early(data["c0-param2"].split(":")[1], start_time):
                break
    ...
    for blog_info in all_blog_info:
        ...
        if start_time:
            if l4_archive_img_finished.is_stamp_early(timestamp, start_time):
                break
        if end_time:
            if l4_archive_img_finished.is_stamp_late(timestamp, end_time):
                continue
        blog_info_dic = {}
        blog_num += 1
...
def run(author_url, start_time, end_time, target_titles, merger_chapter):
    ...
    query_num = 50
    data = l4_archive_img_finished.make_data(author_id, query_num)
    head = l4_archive_img_finished.make_head(author_url)
...
if __name__ == '__main__':
    ...
    author_url = "https://yujochen.lofter.com/"
