...
def get_time_and_title(blog_url, author_page_parse):
    ...
    data = l4_archive_img_finished.make_data(author_id, 50)
    header = l4_archive_img_finished.make_head(author_url)
    blog_id = blog_url.split("/")[-1]
    flag = False
    the_blog_info = ""
    while True:
        page_data = l4_archive_img_finished.post_content(url=archive_url, data=data, head=header)
        # 正则匹配出每条博客的信息
