...
def get_time(blog_url, author_page_parse):
    ...
    data = l4_archive_finished.make_data(author_id, 50)
    header = l4_archive_finished.make_head(author_url)
    blog_id = blog_url.split("/")[-1]
    flag = False
    the_blog_info = ""
    while True:
        page_data = l4_archive_finished.post_content(url=archive_url, data=data, head=header)
        # 正则匹配出每条博客的信息
...
def parse_blogs_info(blogs_urls):
    ...
    for blog_url in blogs_urls:
        ...
        # 过滤博客页面中获取到的图片链接
        imgs_url = l4_archive_finished.fliter_img(imgs_url)
        print(imgs_url)
