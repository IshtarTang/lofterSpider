...
有问题/报错优先提issue，不知道怎么提issue可以直接邮箱找我：ishtartang@163.com，但是不要在提了issue之后又给我发邮件。得说清楚你用的哪个程序，爬的哪个链接，报错长啥样(截图或者复制报错内容)，我debug起来比较方便。

有的时候会懒得更新自述文件，如果自述文件和代码注释冲突 以代码注释为准。
...
先点开文件 login_info.py，这个文件是专门填登录信息的，lofter现在的页面都要求登录，这里填好可以同步到每个程序，不填其他的都跑不起来
...
lofter最近好像加了控制台检测，打开控制台的时候有些请求发不了，所以这里加另个方式2

只要是登录了之后lofter站内任意网页就行，在没打开控制台的情况下刷新，再按F12打开控制台，按Application，按cookies，点开cookies展开后的lofter链接。找你登录方式的 login_key，这里是用手机登录，就找LOFTER-PHONE-LOGIN-AUTH，把右边的值复制下来，粘贴到代码里
...
`tag`：保存一个tag里的内容，只能保存前1200条内容（已经找到能获取更多的办法，但是那得解析移动端端口重写，实在是没时间，将就用吧）
...
tag模式需要的是tag链接，tag链接有5种结尾，分别是 最新(new)、热度总版(total)、月榜(month)、周榜(week)，日榜(date)，这些都可以，放那个获取得就是哪个，浏览器直接点进tag的链接是没有最后一截的，会指向最新。
...
##### 运行过程（这是给我自己的笔记，除非你要手动调运行进度，不然不用看这段）
...
parse_template.py：l9和l10的xpath文字匹配模板
