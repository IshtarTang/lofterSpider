...
感觉这个自述文件有点乱，有些东西也没更新进去，但是我懒得写了，配置项填写以代码里的注释为准。
...
先点开文件 login_info.py，这个文件是专门填登录信息的，这里填好可以同步到每个程序，不填其他的都跑不起来
...
| QQ登录         | LOFTER_SESS（未验证）   |
| 邮箱登录       | NTES_SESS（未验证）     |
| 微博登录       | 未知                    |
| 微信登录       | 未知                    |

博主并没有用QQ登录和邮箱登录的lofter账号，这两个 key 信息来自网友提供，如果有人用了这两种方式，成功或失败都可以向我反馈下，我好确认或者修复。

微博登录和微信登录的Key都还不知道，如果有人需要这两种登录方式需要你配合帮我找找key才行。
...
点开lofter主页或者喜欢页，按F12召唤开发 者工具，按Network，按XHR，往下划拉几下会加载一个文件，点它
...
url：
like1 like2 share模式的url是你个人主页的链接。链接中有的时候有表单，比如 http://www.lofter.com/like?act=qbdashboardlike_20121221_01 ，要把问号和后面的都删掉，变成 http://www.lofter.com/like 。

tag模式需要的是tag链接，tag链接有5种结尾，分别是 最新(new)、热度总版(total)、月榜(month)、周榜(week)，日榜(date)，浏览器直接点进tag的链接是没有最后一截的，会指向最新
...
跟like1不一样，like2爬的是 http://www.lofter.com/like ，这个页面是登录后访问的，须要登录信息，另外，只支持手机登录的，邮箱的登录请求有加密我解不开。手机号就是手机号密码就是密码。开始时间是功能，在功能里说。
...
| 输出等级                           | print_level      | 整型     | /           | 等级1的输出多，0的输出少                                     | 啥时候改都行 | 设为1会把每一条博客的信信息都输出来，debug用的               |
...
获取阶段的实际返回条数比请求条数少，因为你点过喜欢/推荐/tag中曾经存在的内容，有些已经被屏蔽或者作者删除的，也会计算进去，但实际上已经请求不到了，一般来说时间约久消失的条数越多。（我的少了1/10，艹）
...
功能在注释里都写了，不做自定义设置的话把作者主页链接拷到author_url，拿到login_auth填好，运行就行
